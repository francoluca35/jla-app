...
export default function FormVentasPactadas() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      ventaDesdeStock: Boolean(formData.ventaDesdeStock),
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="flex items-center gap-3 px-4 py-2.5 border border-gray-300 rounded-lg mt-6">
            <input
              type="checkbox"
              name="sena"
              checked={formData.sena}
              onChange={handleChange}
              className="w-4 h-4 accent-verdefluor"
            />
            <span className="text-sm text-gray-800">Seña: {formData.sena ? "Sí" : "No"}</span>
          </label>

          <label className="flex items-start gap-3 px-4 py-2.5 border border-gray-300 rounded-lg md:col-span-2">
            <input
              type="checkbox"
              name="ventaDesdeStock"
              checked={formData.ventaDesdeStock}
              onChange={handleChange}
              className="w-4 h-4 accent-verdefluor mt-0.5"
            />
            <span className="text-sm text-gray-800">
              Venta desde stock (producto ya disponible en depósito). El importe total cuenta como ganancia.
            </span>
          </label>
