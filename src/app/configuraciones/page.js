...
export default function ConfiguracionesPage() {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      ...
      try {
        const config = await getBiometricConfig(resolvedUsername);
        setBiometricConfig(config);
      } catch {
        setBiometricConfig({ prompted: false, slots: [false, false, false], credentialIds: [] });
      }
    };
    loadConfig();
  }, []);
  ...
  const handleBorrarBiometricos = async () => {
    ...
    try {
      ...
      const configReseteada = await clearBiometricData(username);
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm divide-y divide-gray-100">
          ...
          <div className="p-5 flex items-center justify-between gap-4">
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg bg-gray-800 text-red-100 text-sm hover:bg-gray-600 transition-colors disabled:opacity-60"
            >
