...
function useIngresos() {
  ...
  const calcularTotal = () => {
    ...
    // Filtra los ingresos con el estado "terminado" y solo los "presupuestos"
    const filteredData = data.clientes.filter((item) => {
      // Solo los "presupuestos" con estado "terminado"
      if (item.problemType === "presupuesto" && item.estado === "terminado") {
        return true;
      }
      return false; // No incluir otros tipos o estados
    });

    // Sumar los montos de los datos filtrados
