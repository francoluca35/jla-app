...
const FormClient = () => {
  ...
  useEffect(() => {
    const now = new Date();
    setDate(now.toISOString()); // Esto guarda la fecha en formato estándar ISO
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (problemType === "arreglo") {
      sertec.push({ tipo: "arreglo", monto: Number(amount) || 0 });
    } else if (problemType === "presupuesto" && paymentOption) {
      sertec.push({ tipo: paymentOption, monto: Number(amount) || 0 });
    }
