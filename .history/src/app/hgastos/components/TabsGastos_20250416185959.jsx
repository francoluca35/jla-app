...
const TabsGasto = () => {
  ...
  // Transforma a formato '15/4/2025'
  const formatoFecha = (fecha) =>
    `${fecha.getDate()}/${fecha.getMonth() + 1}/${fecha.getFullYear()}`;

  const parsearFechaTexto = (texto) => {
    const [dia, mes, año] = texto.split("/").map(Number);
  ...
  const gastosFiltrados = gastos.filter((g) => {
    ...
    const dentroDeRango =
      (!fechaDesde || fechaGasto >= parsearFechaTexto(fechaDesde)) &&
      (!fechaHasta || fechaGasto <= parsearFechaTexto(fechaHasta));
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <input
            type="text"
            placeholder="Desde (ej: 15/4/2025)"
            className="p-2 w-full bg-black rounded"
            value={fechaDesde}
          ...
          <input
            type="text"
            placeholder="Hasta (ej: 15/4/2025)"
            className="p-2 w-full bg-black rounded"
            value={fechaHasta}
