...
export default function GananciasPanel() {
    ...
    if (Array.isArray(clientes)) {
      for (const c of clientes) {
        ...
        if (
          ...
          c.presupuestoGanancia?.materiaPrimaEstado === "calculado"
        ) {
  ...
  const filas = [
    ...
    {
      label: "Presupuestos (MP calculada: seña o pago total)",
      value: resumen.presupuestoNeto,
      hint: "Total presupuesto − materia prima",
    },
    {
      label: "Presupuestos pago total antiguos (sin MP en sistema)",
      value: resumen.presupuestoDirecto,
      hint: "Solo registros viejos sin seguimiento de MP",
    },
    {
  ...
  return (
    <div className="bg-white border border-emerald-200 rounded-2xl p-5 shadow-sm">
      ...
      <ul className="space-y-3 text-sm">
        {filas.map((f) => (
          ...
          >
            ...
            <p className="text-gray-900 font-bold tabular-nums sm:text-right">{formatMoneda(f.value)}</p>
          </li>
        ))}
      </ul>
      <div className="mt-4 pt-4 border-t border-emerald-100 flex items-center justify-between gap-2">
        ...
        <span className="text-lg font-bold text-emerald-800 tabular-nums">{formatMoneda(resumen.total)}</span>
      </div>
    </div>
  );
}
