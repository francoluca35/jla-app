    ...
    const calcularIngresosPorSemanaYMes = () => {
      ...
      const ingresosPorSemana = data.clientes
        .filter((cliente) => {
          ...
          return diffDays <= 7; // Filtro para la semana actual
        })
        .reduce((acc, curr) => acc + (curr.amount || 0), 0);

      // Si no se encuentran ingresos para esta semana, buscamos en el mes anterior
      if (ingresosPorSemana === 0) {
        return {
          ingresosPorSemana: data.clientes
            .filter((cliente) => {
              const date = new Date(cliente.fecha); // Suponiendo que la fecha de ingreso está en 'fecha'
              const today = new Date();
              const diffTime = Math.abs(today - date);
              const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
              return diffDays <= 30; // Filtro para el mes actual
            })
            .reduce((acc, curr) => acc + (curr.amount || 0), 0),
          ingresosPorMes: data.clientes
            .filter((cliente) => {
      ...
      // Si hay ingresos para esta semana, los sumamos para la semana y el mes
