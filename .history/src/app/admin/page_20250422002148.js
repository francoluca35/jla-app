...
export default function AdminAuth() {
  ...
  const handleFingerprintLogin = async () => {
    try {
      // Hacer la solicitud al backend para obtener el challenge
      const response = await fetch("/api/authenticate-fingerprint", {
        method: "POST",
        body: JSON.stringify({ username }), // Enviar solo el username
        headers: { "Content-Type": "application/json" },
      });

      const { challenge } = await response.json();
      console.log("Challenge recibido del backend:", challenge); // Verifica que el challenge esté aquí

      if (!challenge) throw new Error("No se recibió un challenge");

      // Obtener la credencial utilizando WebAuthn
      const credential = await navigator.credentials.get({
        publicKey: {
          challenge: new TextEncoder().encode(challenge), // Usar el challenge recibido
          allowCredentials: [
            {
              type: "public-key",
              id: new TextEncoder().encode(username), // Asegúrate de que el ID de usuario esté correcto
            },
          ],
          timeout: 60000,
        },
      });
      console.log("Credencial obtenida:", credential); // Verifica la credencial

      // Enviar la credencial para autenticarla
      const authResponse = await fetch("/api/authenticate-fingerprint", {
        method: "POST",
        body: JSON.stringify({
          username,
          credential, // Asegúrate de que la credencial se esté enviando correctamente
        }),
        headers: { "Content-Type": "application/json" },
      });

      const authData = await authResponse.json();
      console.log("Respuesta de autenticación:", authData); // Verifica la respuesta
