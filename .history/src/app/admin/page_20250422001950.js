...
export default function AdminAuth() {
  ...
  const handleFingerprintLogin = async () => {
    try {
      ...
      console.log("Challenge recibido del backend:", data.challenge); // Verifica que el challenge esté aquí

      if (!data.challenge) throw new Error("No se recibió un challenge");

      // Usar el challenge para autenticar al usuario con WebAuthn
      const credential = await navigator.credentials.get({
        publicKey: {
          challenge: new TextEncoder().encode(data.challenge), // Challenge recibido del backend
          allowCredentials: [
            {
              type: "public-key",
              id: new TextEncoder().encode(user.webAuthnCredential.id),
            },
          ],
          timeout: 60000,
        },
