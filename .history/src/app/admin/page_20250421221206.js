  ...
  const handleFingerprintLogin = async () => {
    try {
      const response = await fetch("/api/authenticate-fingerprint", {
        method: "POST",
        body: JSON.stringify({ username, credential }), // Enviar el `credential` junto con el `username`
      ...
      const credential = await navigator.credentials.get({
        publicKey: {
          challenge: new TextEncoder().encode(challenge), // Challenge recibido del servidor
          allowCredentials: [
            {
              type: "public-key",
