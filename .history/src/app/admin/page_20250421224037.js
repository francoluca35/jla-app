...
export default function AdminAuth() {
  ...
  const handleFingerprintLogin = async () => {
    try {
      // Primero obtenemos el challenge del servidor
      const challengeResponse = await fetch("/api/authenticate-fingerprint", {
        method: "POST",
      ...
      const credential = await navigator.credentials.get({
        publicKey: {
          ...
          allowCredentials: [
            {
              type: "public-key",
              id: new TextEncoder().encode("user's credential ID"),
            },
          ],
          timeout: 60000,
        },
      ...
      const authResponse = await fetch("/api/authenticate-fingerprint", {
        ...
        body: JSON.stringify({
          username,
          credential, // Aquí estamos enviando la credencial
        }),
        headers: { "Content-Type": "application/json" },
      });
