...
export default function AdminAuth() {
  ...
  const handleFingerprintLogin = async () => {
    try {
      ...
      // Crear la credencial con el challenge recibido
  ...
  const handleFingerprintLogin = async () => {
    try {
      ...
      const credential = await navigator.credentials.get({
        publicKey: {
          ...
          allowCredentials: [
            {
              type: "public-key",
              id: new TextEncoder().encode(data.webAuthnCredential.id), // Aquí debes usar el id de la credencial
            },
          ],
          timeout: 60000,
        },
      ...
      const authResponse = await fetch("/api/authenticate-fingerprint", {
        ...
        body: JSON.stringify({
          username,
          credential, // Asegúrate de enviar todo el objeto `credential`
        }),
        headers: { "Content-Type": "application/json" },
      });
