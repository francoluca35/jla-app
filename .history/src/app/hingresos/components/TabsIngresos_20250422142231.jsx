...
function TabsIngresos() {
  ...
  const datos = obtenerFiltrados().filter((item) => {
    // Solo filtrar los "presupuestos" con estado "terminado"
    if (item.problemType === "presupuesto" && item.estado !== "terminado") {
      return false; // Excluir "presupuestos" que no están "terminados"
    }
    return true; // Incluir todos los demás ingresos
  });
  ...
  const handleEditar = () => {
    ...
    // Asegúrate de que estamos copiando correctamente los datos del item seleccionado
    setForm({
      clientName: item.clientName || "",
      branch: item.branch || "",
      problemType: item.problemType || "",
      paymentOption: item.paymentOption || "",
      amount: item.amount || "", // Asegurarte de que amount es el correcto
    });
  };

  const confirmarEdicion = async () => {
    // Obtener el monto modificado y asegurarse de que es un número válido
    const updatedData = {
      ...form,
      amount: parseFloat(form.amount), // Asegurarse de que el monto es un número
    };
    ...
    refetch(); // Esto asegura que los datos sean recargados después de editar
    setModoEditar(false);
    setEditando(null);
    setSeleccionados([]);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center gap-4 mb-6 flex-wrap justify-center">
        ...
        >
          Todos {/* Mostrar total combinado aquí */}
        </button>
        <button
          key="servicioT"
        ...
        <table className="w-full border border-green-700 bg-black bg-opacity-60 text-white rounded">
          ...
          <tfoot>
            <tr className="bg-green-600 text-black font-bold text-center">
              ...
              <td className="p-3 border border-green-700">
                ${calcularTotal()}${calcularTotalCombinado()}
              </td>
            </tr>
          </tfoot>
        </table>
